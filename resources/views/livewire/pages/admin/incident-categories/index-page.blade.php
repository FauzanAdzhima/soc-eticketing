<div class="space-y-4">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <flux:heading size="xl">Daftar kategori insiden</flux:heading>
        @can('incident-category.create')
            <flux:button size="sm" variant="primary" wire:click="openCreateModal">Tambah kategori</flux:button>
        @endcan
    </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-700">
                <thead class="bg-zinc-50 dark:bg-zinc-800/80">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left font-semibold text-zinc-700 dark:text-zinc-200">
                            <button type="button" wire:click="sortByColumn('slug')" class="inline-flex items-center gap-1 hover:text-zinc-900 dark:hover:text-white">
                                Slug
                                @if ($sortBy === 'slug')
                                    <span class="text-xs font-normal">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                                @endif
                ...
                <tbody class="divide-y divide-zinc-200 bg-white dark:divide-zinc-700 dark:bg-zinc-900">
                        ...
                        <tr wire:key="cat-{{ $cat->id }}" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                            ...
                            <td class="whitespace-nowrap px-3 py-2 text-right">
                                <div class="inline-flex flex-wrap justify-end gap-1">
                                    ...
                                    @endcan
                                    @can('incident-category.delete')
                                        <flux:button size="sm" variant="danger" wire:click="openDeleteModal({{ $cat->id }})">Hapus</flux:button>
                                    @endcan
                                </div>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-zinc-500 dark:text-zinc-400">Tidak ada kategori.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    @if ($showFormModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 p-4 backdrop-blur-sm">
            <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{{ $editingId ? 'Ubah kategori' : 'Tambah kategori' }}</h3>
                <p class="mt-1 text-sm text-zinc-500 dark:text-zinc-400">Slug kosong akan dibuat otomatis dari nama.</p>
                ...
                <div class="mt-4 space-y-4">
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Slug (opsional)</label>
                        <input type="text" wire:model="formSlug" placeholder="contoh: phishing-alert"
                            class="w-full rounded-lg border border-zinc-300 px-3 py-2 font-mono text-sm outline-none focus:border-zinc-500 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
                        @error('formSlug')
                            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <flux:button size="sm" variant="primary" wire:click="saveCategory">Simpan</flux:button>
                </div>
            </div>
        </div>
    @endif
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/40 p-4 backdrop-blur-sm">
            <div class="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-2xl dark:border-zinc-700 dark:bg-zinc-900">
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Hapus kategori</h3>
                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-300">Kategori yang masih dipakai tiket tidak dapat dihapus.</p>
                <div class="mt-6 flex justify-end gap-2">
                    <flux:button size="sm" variant="ghost" wire:click="closeDeleteModal">Batal</flux:button>
                    <flux:button size="sm" variant="danger" wire:click="deleteCategory">Hapus</flux:button>
                </div>
            </div>
        </div>
    @endif
